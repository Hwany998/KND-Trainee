...
# 기준 데이터에서 결측 행을 삭제한 제거 버전
제거판 = 기준.dropna()
print(제거판.shape) # (110, 20)

# 기준 데이터의 결측을 중앙값으로 채운 대체 버전
...
# 결측 비율 높은 컬럼을 제거하고 나머지는 중앙값으로 채우기 / 대체판 이어 사용

# 처리 후 남은 결측과 크기를 확인하고 파일로 저장
print(대체판.isna().sum().sum()) # 0
대체판.to_csv('csv/15_02_사출성형_공정_clean.csv', index=False, encoding='utf-8')
