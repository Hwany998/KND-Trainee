...
df = pd.read_csv('csv/15_02_사출성형_공정.csv', encoding='utf-8')
...
# 비율이 40% 이상 기준 넘은 NaN으로 채워진 컬럼 목록 이름만 목록 뽑기
...
비교['손실률'] = ((1 - 비교['행'] / len(df)) * 100) .round(2)   #이게 고급코드
...
# 위 코드는 고급기술 - DF의 더 깊은 이해와 경험 필요
# 그냥 개별 3가지 항목들을 따로따로 계산시켜 출력해도 괜찮음. 단, 도전은 해볼 것.
