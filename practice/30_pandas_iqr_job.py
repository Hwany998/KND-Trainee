...
CD = 'csv/16_diecasting.csv'
WD = 'csv/16_welding.csv'
...
# 원본과 제거 후의 행 수를 비교
print(len(df), len(normal))  # 202 196

# 제거 후 평균을 구해 변화 확인
print(round(df['사이클타임'].mean(), 2)) # 64.75
print(round(normal['사이클타임'].mean(), 2)) # 27.28
...
# 상한보다 큰 값은 상한으로 내리기
fix = df['사이클타임'].clip(lower=lower, upper=upper)

# 보정 후 최솟값·최댓값·평균 확인
print(round(fix.min(), 2), round(fix.max(), 2))   # 20.6 58.61
print(round(fix.mean(), 2)) # 28.28
...
# 실린더압력 열에 대해 원본 / 행 제거 / clip 보정 / 중앙값 채움 상태의 평균 변화를 정밀 대조
...
full = df['실린더압력'].mask(m).fillna(df['실린더압력'].mask(m).median())
...
print(round(full.mean(), 2)) # 238.05
...
wf = pd.read_csv(WD)    #16_Welding.csv
c = '통전전류'
